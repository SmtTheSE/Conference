...
def predict_price():
    ...
    try:
        ...
        # NLP Generation - PRODUCTION GRADE
        yield_pct = (rent * 12 / price) * 100
        interest_status = "Emerging" if demand_score > 70 else "Stable" if demand_score > 40 else "Stagnant"
        
        if demand_score > 80 and yield_pct > 6.0:
             insight_text = f"🔥 HIGH-EFFICIENCY GAP: {data['location']} shows severe demand outstripping price. Alpha Potential: High. Net Initial Yield: {yield_pct:.2f}%."
        elif yield_pct > 8.0:
             insight_text = f"Yield-Centric Opportunity. High rental velocity detected in {data['location']}. Ideal for institutional buy-to-rent. Est. {yield_pct:.2f}% GROSS."
        elif demand_score < 30:
             input_type = data.get('property_type', 'Asset')
             insight_text = f"Market Saturation Warning. {input_type} inventory in {data['location']} is currently in oversupply. Monitoring for price correction."
        else:
             insight_text = f"Fair Tactical Value. {data['location']} maintains {interest_status} market equilibrium. Benchmark yield aligns with regional medians at {yield_pct:.2f}%."
