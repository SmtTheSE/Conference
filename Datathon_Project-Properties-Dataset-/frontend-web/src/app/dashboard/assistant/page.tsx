...
export default function CulturalAssistant() {
    ...
    const [loadingText, setLoadingText] = useState("Consulting Synergia 2026 ground truth...");

    useEffect(() => {
        if (!isLoading) return;
        const messages = [
            "Consulting Synergia 2026 ground truth...",
            "Analyzing Pan-Asian legal records...",
            "Calculating regional yield profiles...",
            "Synthesizing cultural nuances...",
            "Finalizing investment guidance..."
        ];
        let i = 0;
    ...
    return (
        <div className="animate-in fade-in duration-500 flex flex-col h-[calc(100vh-200px)]">
            <div className="mb-6">
                ...
                <p className="text-text-muted">Query the phi3:mini local LLM for pan-Asian property law and cultural nuance.</p>
            </div>
                ...
                `}>
                        ...
                        {isLoading && (
                            <div className="flex justify-start">
                                <div className="bg-white border border-border-light text-text-muted rounded-r-lg rounded-tl-lg shadow-sm px-5 py-3.5 flex flex-col gap-2">
                                    <div className="flex gap-2 items-center">
                                        <div className="w-2 h-2 rounded-full bg-asean-navy animate-bounce" style={{ animationDelay: '0ms' }}></div>
                                        <div className="w-2 h-2 rounded-full bg-asean-navy animate-bounce" style={{ animationDelay: '150ms' }}></div>
                                        <div className="w-2 h-2 rounded-full bg-asean-navy animate-bounce" style={{ animationDelay: '300ms' }}></div>
                                    </div>
                                    <div className="text-[10px] font-bold uppercase tracking-widest text-asean-navy/50 animate-pulse">
                                        {loadingText}
                                    </div>
                                </div>
                            </div>
