...
class PricingModel:
    ...
    def prepare_data(self):
        ...
        if df.empty or 'transaction_type' not in df.columns:
            print("Warning: No data or 'transaction_type' column missing. Skipping data preparation.")
    ...
    def train(self):
        ...
        if X.empty:
            print("Skipping training: No source data available.")
            return None, df
...
class RentalModel:
    ...
    def calculate_baseline_yield(self):
        ...
        try:
            df = self.loader.load_unified_data()
            if df.empty or 'transaction_type' not in df.columns or 'country' not in df.columns:
                return 0.05

            vn_data = df[df['country'] == 'Vietnam']
            
            # Group by location to get median Price and Rent
            if vn_data.empty: return 0.05
...
class YieldCurveModel:
    ...
    def train(self):
        ...
        if df.empty or 'country' not in df.columns:
            print("No data available for Yield Curve Model training.")
            return
